...
- 이동생성자 
    ...
    - **&&(오른값 참조)와 std::move()는 세트**
    - &&는 이동이 가능한 객체를 받아들이는 역할을 하고,
    - std::move()는 기존 객체를 "이동 가능 상태"로 변환하는 역할을 한다, 즉 **객체를 && 형으로 형변환**한다.

    - 동적할당된 자료는 원본값이 유실될 수 있고, 기본 자료형 이나 스택에 저장된 녀석들은 원본값이 유지된다.
